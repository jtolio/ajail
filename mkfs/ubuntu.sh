#!/bin/bash
#
# mkubuntu.sh - Create an Ubuntu root filesystem using debootstrap
#
# This script must be run as root (sudo ./mkubuntu.sh ...)
...
Create a minimal Ubuntu root filesystem using debootstrap.

Options:
  -s, --suite SUITE       Ubuntu suite (default: $SUITE)
                          Examples: noble, jammy, oracular, plucky
  -m, --mirror URL        Ubuntu mirror (default: $MIRROR)
...
Examples:
  sudo $0 /tmp/myubuntu
  sudo $0 -s jammy -p vim,git /tmp/myjammy
...
echo
echo "Ubuntu rootfs created successfully at: $OUTPUT_DIR"
